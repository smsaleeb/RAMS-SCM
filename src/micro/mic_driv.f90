...
if(ncall == 0) then
   ...
   if(iprntstmt>=1 .and. print_msg) then
    print*,' '
    print*,'***************************************************************'
    print*,'Microphysics init and lookup table setup on first timestep'
    print*,'***************************************************************'
    print*,' '
   endif
   CALL micinit ()
...
do j = ja,jz
   do i = ia,iz
      ...
      !if(i+mi0(ngr)==15 .and. j+mj0(ngr)==15) then
      ! do k=2,39,2
      !  print'(5i4,1f10.1,2X1f10.5)',k,i,ia,j,ja,time,radiate_g(ngr)%swdn(k,i,j)
      ! enddo
      !endif
   enddo
enddo
...
! Compute pressure, temperature, moisture, and "sa" for vapor diffusion
! based on theta-il and hydrometeor mixing ratios (Walko et al 2000)
 CALL thrmstr (m1,k1,k2,thp(1),theta(1),rtp(1),rv(1) &
              ,pressure(1)                           &
              ,pp(1),pi0(1)                          &
              )

! Compute general terms for collisions, vapor diffusion, saturation, ice habits.
...
do lcat = 1,8
   if (jnmb(lcat) .ge. 1) then
      CALL enemb (m1,k1(lcat),k2(lcat),lcat,dn0(1))
   endif
enddo
...
if(ivapdiff==1)then
! Pre-compute "s" variables for vapor diffusion (Walko et al 2000)
do lcat = 1,8
   if (jnmb(lcat) .ge. 1) then
      CALL diffprep (m1,lcat,k1(lcat),k2(lcat),rv(1),dn0(1))
   endif
enddo
! Update diagnostic water vapor for vapor diffusion / evaporation
 CALL vapdiff (m1,k1(11),k2(11),rv(1))
! Calculate vapor diffusion / evaporation in order of species given by (mivap)
do icv = 1,8
   lcat = mivap(icv)
   if (jnmb(lcat) .ge. 1) then
...
enddo
! Pristine ice to snow transfer after vapor diffusion / sublimation
if (jnmb(4) .ge. 1) then
   CALL psxfer (k1(3),k2(3),k1(4),k2(4),i,j)
endif
! Update temperature and moisture following vapor growth / evaporation
 CALL newtemp (m1,k1(11),k2(11),rv(1),theta(1))
endif !(if vapor diffusion / evaporation)
...
if(iautoaccret==1)then
if (jnmb(2) .ge. 1) then
   CALL auto_accret (m1,k1(1),k2(1),k1(8),k2(8),dn0(1),dtlt)
endif
endif !(if auto-conversion / accretion)
...
endif !(if bin-emulating cldrime)
...
enddo
endif !(if self collection pristine ice / snow)
...
endif !(if pristine-ice snow collisions)

! Ice-ice collisions
if(icolliceice==1)then
do icx = 1,9
   mc1 = mcat1(icx,1)
   mc2 = mcat1(icx,2)
   mc3 = mcat1(icx,3)
...
enddo
endif !(if ice ice collisions)

! Ice-cloud collisions (bulk method)
if(icldrime==1)then
if(irime==0) then
do jcat = 1,8,7
 do lcat = 4,7
...
if (jnmb(1) .ge. 3) CALL enemb (m1,k1(1),k2(1),1,dn0(1))
if (jnmb(8) .ge. 3) CALL enemb (m1,k1(8),k2(8),8,dn0(1))
endif !(if cldnuc)
...
if (jnmb(3) .ge. 3) CALL enemb (m1,k1(3),k2(3),3,dn0(1))
if (jnmb(1) .ge. 3) CALL enemb (m1,k1(1),k2(1),1,dn0(1))
if (jnmb(8) .ge. 3) CALL enemb (m1,k1(8),k2(8),8,dn0(1))
endif !(if icenuc)
