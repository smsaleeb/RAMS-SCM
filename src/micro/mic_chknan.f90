...
do k = 1,mzp
   ...
   if(level>=2)then
     if(icloud >=1 .and. micro_g(ngrid)%rcp(k,i,j)<0.0) mprtflg=1
   endif
   if(level==3)then
 ...
 !*******************************************************************
 if(mprtflg==1)then
    print*,'Negative Condensate MICRO (ngrid,k,i,j):',i,j!&
 ...
 if(prtflg==1)then
    print*,'NAN Check (ngrid,k,j,i)',ngrid,k,i,j !i+mi0(ngrid),j+mj0(ngrid)
