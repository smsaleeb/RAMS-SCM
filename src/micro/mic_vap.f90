...
!OVER WHOLE COLUMN
do k = 1,m1
!SaleebySCM:Replace pitot,press using input pressure instead of Exner(PI) as 
!other models may output pressure rather than Exner function (PI0 + PP)
   if(ipress==0)then
    pitot(k) = pi0(k) + pp(k)
    press(k) = p00 * (pitot(k) * cpi) ** cpor
   elseif(ipress==1)then
    pitot(k) = ((pressure(k)*p00i)**rocp)*cp
    press(k) = pressure(k)
   endif
...
!##############################################################################
Subroutine vapflux (m1,lcat,k1,k2,rv)

use micphys

implicit none

integer :: m1,lcat,k,k1,k2,if1,if4
real :: rxx
real, dimension(m1) :: rv
